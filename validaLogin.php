...
$sql .= "FROM logtentativa WHERE ip = ? and DATE_FORMAT(data_hora,'%Y-%m-%d') = ? AND bloqueado = ?";
...
$sql = 'SELECT idUsuario, nome,login, senha, email FROM usuarios WHERE login = ? AND status = ? LIMIT 1';
$stm = $conexao->prepare($sql);
...
if(!empty($retorno) && password_verify($senha, $retorno->senha)):
	...
	$sql = 'INSERT INTO logtentativa (ip, login,senha, origem, bloqueado) VALUES ( ?, ?, ?, ?, ?)';
	$stm = $conexao->prepare($sql);
	$stm->bindValue(1, $_SERVER['REMOTE_ADDR']);
